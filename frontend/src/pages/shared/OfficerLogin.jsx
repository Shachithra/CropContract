...
export default function OfficerLogin() {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      const u = await login(api, data.email, data.password)
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center bg-cream">
      ...
      <div className="flex-1 flex flex-col items-center px-6 pb-8 w-full max-w-md">
        ...
        {/* Form header */}
        <h2 className="font-display text-xl font-bold text-paddy mt-8 mb-6 self-start">Agricultural Officer Login</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4">
          <div>
            ...
            <input
              id="email"
              type="email"
              placeholder="e.g., officer@demo.lk"
        ...
        <p className="mt-6 text-sm text-text-muted">
          ...
          <Link to="/register" className="font-semibold text-paddy underline underline-offset-2 hover:text-turmeric transition">
            Register
          </Link>
        </p>
      </div>
